...
santa_beard_img = cv2.imread("assets/santa_beard.png", cv2.IMREAD_UNCHANGED) if os.path.exists("assets/santa_beard.png") else None
santa_cap_img = cv2.imread("assets/santa_cap.png", cv2.IMREAD_UNCHANGED) if os.path.exists("assets/santa_cap.png") else None
reindeer_nose_img = cv2.imread("assets/reindeer_nose.png", cv2.IMREAD_UNCHANGED) if os.path.exists("assets/reindeer_nose.png") else None
reindeer_sunglasses_img = cv2.imread("assets/reindeer_sunglasses.png", cv2.IMREAD_UNCHANGED) if os.path.exists("assets/reindeer_sunglasses.png") else None
reindeer_antlers_img = cv2.imread("assets/reindeer_antlers.png", cv2.IMREAD_UNCHANGED) if os.path.exists("assets/reindeer_antlers.png") else None
bunny_ears_img = cv2.imread("assets/bunny_ears.png", cv2.IMREAD_UNCHANGED) if os.path.exists("assets/bunny_ears.png") else None
egg_img = cv2.imread("assets/egg.png", cv2.IMREAD_UNCHANGED) if os.path.exists("assets/egg.png") else None
...
def pool2d(A, kernel_size, stride, padding, pool_mode="max"):
    ...
    return A_w.max(axis=(1, 2)).reshape(output_shape) if pool_mode == "max" else A_w.mean(axis=(1, 2)).reshape(output_shape)
...
def process_landmarks(boxes, results):
    return [(result.reshape(-1, 2) * box[2:] + box[:2]).astype(np.int32) for (_, box), result in zip(boxes, results)]

def process_emotions(results):
    return [EMOTION_CLASSES[np.argmax(result)] for result in results] if results else []

def draw_mask(img, mask_img, center, size, scale=1.0, offset_coeffs=(0.5, 0.5)):
    if mask_img is None: return
    ...
    x1, y1 = center[0] - int(mask_img.shape[1] * offset_coeffs[0]), center[1] - int(mask_img.shape[0] * offset_coeffs[1])
    ...
    if 0 < x2 < img.shape[1] and 0 < y2 < img.shape[0] or 0 < x1 < img.shape[1] and 0 < y1 < img.shape[1]:
        crop = img[max(0, y1):min(y2, img.shape[0]), max(0, x1):min(x2, img.shape[1])]
        mask_crop = mask_img[max(0, -y1):max(0, -y1) + crop.shape[0], max(0, -x1):max(0, -x1) + crop.shape[1]]
        alpha = mask_crop[:, :, 3:4] / 255.0
        crop[:] = (alpha * mask_crop[:, :, :3] + (1 - alpha) * crop).astype(np.uint8)
    ...
    for pose in poses:
        ...
        for i, j in DEFAULT_SKELETON:
            if i < len(points_scores) and j < len(points_scores) and points_scores[i] > point_score_threshold and points_scores[j] > point_score_threshold:
                cv2.line(img_limbs, tuple(points[i]), tuple(points[j]), (0, 0, 0), out_thickness, cv2.LINE_AA)
                cv2.line(img_limbs, tuple(points[i]), tuple(points[j]), (255, 255, 255), in_thickness, cv2.LINE_AA)
        # Draw joints
        for p, v in zip(points, points_scores):
            if v > point_score_threshold:
                cv2.circle(img_limbs, tuple(p), 1, (0, 0, 0), 2 * out_thickness, cv2.LINE_AA)
                cv2.circle(img_limbs, tuple(p), 1, (255, 255, 255), 2 * in_thickness, cv2.LINE_AA)
        cv2.addWeighted(img, 0.3, img_limbs, 0.7, 0, dst=img)

    # Use face landmarks if available, otherwise use pose points for pumpkin overlay
    ...
    if not faces:  # Fallback to pose-based face detection
        for pose in poses:
            ...
            if (points_scores[left_eye] > point_score_threshold and points_scores[right_eye] > point_score_threshold and
                ...
                face_crop[:] = (alpha_pumpkin * pumpkin_face[:, :, :3] + (1 - alpha_pumpkin) * face_crop).astype(np.uint8)

    return img

def draw_santa(img, faces, landmarks, emotions):
    if not faces: return img
    detections = sorted(zip(faces, landmarks, emotions), key=lambda x: x[0][1][2] * x[0][1][3])
    for (score, box), lm, emo in detections[:-1]:
        draw_mask(img, reindeer_antlers_img, np.mean(lm[13:17], axis=0, dtype=np.int32), box[2:], scale=1.8, offset_coeffs=(0.5, 1.1))
        draw_mask(img, reindeer_nose_img, lm[4], box[2:], scale=0.25)
        cv2.putText(img, EMOTION_MAPPING[emo], tuple(lm[4] - [20, 20]), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 196), 1, cv2.LINE_AA)
    (_, box), lm, _ = detections[-1]
    draw_mask(img, santa_beard_img, lm[5], box[2:], offset_coeffs=(0.5, 0))
    draw_mask(img, santa_cap_img, np.mean(lm[13:17], axis=0, dtype=np.int32), box[2:], scale=1.5, offset_coeffs=(0.56, 0.78))
    return img

def draw_easter(img, poses, faces, landmarks):
    for pose in poses:
        torso_center = np.mean(pose[5:7], axis=0, dtype=np.int32) if pose[5][2] > 0.25 and pose[6][2] > 0.25 else None
        if torso_center is not None:
            draw_mask(img, egg_img, torso_center, (100, 150), scale=1.8)
    for (_, box), lm in zip(faces, landmarks):
        draw_mask(img, bunny_ears_img, np.mean(lm[13:17], axis=0, dtype=np.int32), box[2:], scale=1.5, offset_coeffs=(0.5, 1.0))
    return img
...
def load_model(model_name, precision, device):
    ...
    if not model_path.exists():
        ...
        utils.download_file(model_url + model_name + '.xml', model_path.name, model_path.parent)
        utils.download_file(model_url + model_name + '.bin', model_path.with_suffix('.bin').name, model_path.parent)
    core = ov.Core()
...
def run_demo(source, theme, model_precision, device, flip):
    ...
    pose_model = load_model("human-pose-estimation-0001", model_precision, device)
    face_model = load_model("face-detection-0205", model_precision, device)
    landmark_model = load_model("facial-landmarks-35-adas-0002", model_precision, device)
    emotion_model = load_model("emotions-recognition-retail-0003", model_precision, device)

    pose_input_shape = (456, 256)  # Corrected: width, height for cv2.resize to match [1, 3, 256, 456]
    ...
    if not cap.isOpened():
        print(f"Warning: Cannot open webcam {source}. Falling back to default VideoPlayer behavior.")
    player = utils.VideoPlayer(source, flip=flip, fps=30, size=(1920, 1080))
    player.start()

    cv2.namedWindow("Themed Demo", cv2.WINDOW_GUI_NORMAL)
    cv2.setWindowProperty("Themed Demo", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    ...
    while True:
        ...
        pose_input = cv2.resize(frame, pose_input_shape, interpolation=cv2.INTER_AREA).transpose((2, 0, 1))[np.newaxis, ...]
        pose_results = pose_model([pose_input])
        poses, _ = process_poses(frame, pose_results[pose_model.output("Mconv7_stage2_L1")], pose_results[pose_model.output("Mconv7_stage2_L2")], pose_model, decoder)

        # Face detection and processing
        face_input = cv2.resize(frame, face_input_shape, interpolation=cv2.INTER_AREA).transpose((2, 0, 1))[np.newaxis, ...]
        faces = process_faces(frame, face_model([face_input])[face_model.output(0)], face_input_shape[1], face_input_shape[0])
        patches = [frame[box[1]:box[1] + box[3], box[0]:box[0] + box[2]] for _, box in faces]
        landmarks = process_landmarks(faces, [landmark_model([cv2.resize(p, tuple(landmark_input_shape), interpolation=cv2.INTER_AREA).transpose((2, 0, 1))[np.newaxis, ...]])[landmark_model.output(0)].squeeze() for p in patches])
        emotions = process_emotions([emotion_model([cv2.resize(p, tuple(emotion_input_shape), interpolation=cv2.INTER_AREA).transpose((2, 0, 1))[np.newaxis, ...]])[emotion_model.output(0)].squeeze() for p in patches])
...
if __name__ == "__main__":
    ...
    run_demo(args.stream, args.theme, args.model_precision, args.device, args.flip)